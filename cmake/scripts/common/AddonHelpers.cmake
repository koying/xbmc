...
macro(add_cpack_workaround target version ext)
  ...
  add_custom_command(TARGET addon-package POST_BUILD
                     ...
                     COMMAND ${CMAKE_COMMAND} -E copy ${CPACK_PACKAGE_DIRECTORY}/addon-${target}-${version}-${PLATFORM}.${ext} ${PACKAGE_DIR}/${target}-${version}-${PLATFORM}.${ext})
endmacro()
...
macro (build_addon target prefix libs)
  ...
  if(EXISTS ${PROJECT_SOURCE_DIR}/${target}/addon.xml.in)
    ...
    if(CORE_SYSTEM_NAME STREQUAL windows)
      ...
      if(_X86_)
        set(PLATFORM ${PLATFORM}-i686)
      elseif(_AMD64_)
        set(PLATFORM ${PLATFORM}-x86_64)
      else()
    ...
    elseif(CORE_SYSTEM_NAME STREQUAL android)
      if (CPU MATCHES "v7a")
        set(PLATFORM ${PLATFORM}-armv7)
      elseif (CPU MATCHES "arm64")
        set(PLATFORM ${PLATFORM}-aarch64)
  ...
  if(PACKAGE_ZIP OR PACKAGE_TGZ)
    ...
    list(APPEND CPACK_COMPONENTS_ALL ${target}-${${prefix}_VERSION}-${PLATFORM})
    # Pack files together to create an archive
    install(DIRECTORY ${target} ${CMAKE_CURRENT_BINARY_DIR}/${target} DESTINATION ./
                                COMPONENT ${target}-${${prefix}_VERSION}-${PLATFORM}
